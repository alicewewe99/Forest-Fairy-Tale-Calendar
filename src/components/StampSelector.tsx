...
export const StampSelector: React.FC<StampSelectorProps> = ({
  ...
  const row1Stamps = AVAILABLE_STAMPS.slice(0, 6);
  const row2Stamps = AVAILABLE_STAMPS.slice(6);
  ...
  return (
    <div className="bg-[#FAF0CA] border border-[#E0A96D] rounded-2xl p-2 sm:p-2.5 shadow-inner">
      ...
      {/* 2-Row Stamps Display with synchronized scrolling */}
      <div className="overflow-x-auto pb-1 no-scrollbar" id="stampSelectorScroll">
        <div className="flex flex-col gap-1.5 min-w-max">
          {/* Row 1: Function Controls & Common Stamps */}
          <div className="flex items-center gap-1.5" id="stampSelectorRow1">
            {/* Clear / Eraser Button (At the very front) */}
            <button
              id="stampBtn-CLEAR"
          ...
          {/* Row 2: Nature & Lifestyle Stamps */}
          <div className="flex items-center gap-1.5" id="stampSelectorRow2">
            {row2Stamps.map((s) => {
              const isActive = selectedStamp === s.id;
              return (
                <button
